...
public:
    ...
    void printInW(WINDOW *win, int h, int w, std::string text);
...
void gameWindow::instructionsForTulip(WINDOW *win){
    ...
    mvwprintw(win, height/4+9, width/5, "press 'e' to continue");
...
void gameWindow::printInW(WINDOW *win, int h, int w, std::string text){
    mvwprintw(win, h, w, text.data());
}
